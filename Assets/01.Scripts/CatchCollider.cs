...
public class CatchCollider : MonoBehaviour {
    ...
    void Update() {
        ...
        //if (!trigger.GetState(anyHand) && rightPiece) {
        if (trigger.GetStateUp(right) && rightPiece) {
            puzzlePices.transform.parent = transform;
            Vector3 offset = new Vector3(0.0f, 0.0f, -0.3f);
            puzzlePices.transform.localPosition = Vector3.zero + offset;
